...
(function(){
  ...
  async function drawOverlay(ctx,canvas,ev,config,helpers={}){
    const c=config&&Object.keys(config).length?config:cfgOf(ev);
    const ov=c.overlay||{},w=canvas.width,h=canvas.height;
    ...
    if(banner.enabled!==false){
      ...
      if(type==='solid'){ctx.fillStyle=alpha(color,op);ctx.fillRect(0,by,w,bh)}
        ...
        const g=ctx.createLinearGradient(0,by,0,h);g.addColorStop(0,alpha(color,0));g.addColorStop(.28,alpha(color,op*.7));g.addColorStop(1,alpha(color,op));ctx.fillStyle=g;ctx.fillRect(0,by,w,bh);
      }
    ...
    const pad=w*Math.max(.025,Math.min(.10,Number(ov.padding_pct||4.5)/100)),maxW=w-pad*2;
    const title=ov.title||{},sub=ov.subtitle||{},line=ov.line||{};
    const align=title.align||'left',x=align==='center'?w/2:align==='right'?w-pad:pad;
    const titleY=by+bh*.43,subY=by+bh*.66,lineY=by+bh*.84;
    if(title.enabled!==false)drawMulti(ctx,textValue(title,ev,'title'),x,titleY,title,maxW);
    if(sub.enabled!==false){const a=sub.align||align,xx=a==='center'?w/2:a==='right'?w-pad:pad;drawMulti(ctx,textValue(sub,ev,'subtitle'),xx,subY,{...sub,align:a,size_pct:sub.size_pct||3.2,weight:sub.weight||800,color:sub.color||ev.accent||'#d9b56d'},maxW)}
    if(line.enabled!==false){
      let txt=textValue(line,ev,'overlay');
      if(line.include_date!==false&&helpers.photoEventDayText){const d=helpers.photoEventDayText();if(d)txt=txt?txt+' · '+d:d}
  ...
  window.FTS_STUDIO_V56={catalog,filterInfo,applyFilter:pxFilter,filterCss,drawOverlay,fontStack,pumpkinFrame};
})();