...
rm -f "$DIST/FTS-Printer-macOS-v0.3.2-native-only.zip" "$DIST/FTS-Printer-macOS-v0.3.2-native-only.dmg"
ditto -c -k --sequesterRsrc --keepParent "$APP" "$DIST/FTS-Printer-macOS-v0.3.2-native-only.zip"
DMG="$DIST/FTS-Printer-macOS-v0.3.2-native-only.dmg"
rm -f "$DMG"
