...
const CACHE='fts-selfie-v12-galleryfiltersdownloadposter-20260918';
...
async function register(item){
  const r=await fetch(`${cfg.supabaseUrl}/rest/v1/rpc/fts_register_photo`,{
    method:'POST',
    headers:{'apikey':cfg.publishableKey,'Authorization':'Bearer '+cfg.publishableKey,'Content-Type':'application/json'},
    body:JSON.stringify({p_event_token:item.eventToken,p_original_path:item.originalPath,p_designed_path:item.designedPath})
  });
