...
async function register(item){const r=await fetch(`${cfg.supabaseUrl}/rest/v1/rpc/fts_register_photo_v11`,{method:'POST',headers:{'apikey':cfg.publishableKey,'Authorization':'Bearer '+cfg.publishableKey,'Content-Type':'application/json'},body:JSON.stringify({p_event_token:item.eventToken,p_original_path:item.originalPath,p_designed_path:item.designedPath,p_guest_session_id:item.guestSessionId||null,p_captured_at:new Date(item.createdAt||Date.now()).toISOString()})});if(!r.ok)throw new Error(await r.text())}
...
async function flush(){const items=await allItems();for(const item of items){try{await uploadObject(item.originalPath,item.originalBlob,item.originalType);await uploadObject(item.designedPath,item.designedBlob,'image/jpeg');await register(item);await del(item.id);await reportIssue(item,'background_upload_failed','Hintergrund-Upload wurde erfolgreich nachgeholt.','',true)}catch(e){console.warn('FTS background upload stopped',e);await reportIssue(item,'background_upload_failed','Hintergrund-Upload konnte nicht abgeschlossen werden.',e,false);throw e}}const clients=await self.clients.matchAll({includeUncontrolled:true,type:'window'});clients.forEach(c=>c.postMessage({type:'FTS_QUEUE_FLUSHED'}))}
